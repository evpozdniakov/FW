$.extend({
	...
	evUniqId: function(){
		while(true){
			var id='evId'+Math.random().toString().substr(2);
...
$.fn.extend({
	...
	evMask: function(hash){
		// hash.re — regular expression, e.g. /^[0-9]{4}$/
		// hash.editingClass - string, className to add to html-element while user types (optional, default 'editing')
		// hash.matchClass - string, className to add to html-element when regular expression matches (optional, default 'match')
		// hash.notmatchClass - string, className to add to html-element when regular expression doesn't match (optional, default 'notmatch')
		// hash.checkInterval - number, interval (in milliseconds) to check if field value matches or not (optional, default 500)
		// hash.remainEditing - number, interval (in milliseconds) to consider the field editable (optional, default 3000)
		this.each(function(){
			hash.editingClass=(hash.editingClass || 'editing');
			hash.matchClass=(hash.matchClass || 'match');
			hash.notmatchClass=(hash.notmatchClass || 'notmatch');
			hash.checkInterval=parseInt(hash.checkInterval || '500');
			hash.remainEditing=parseInt(hash.remainEditing || '3000');
			var $wrap, wrap_id, $field, back_value, actual_value;
			$wrap=$(this);
			...
			if(!C['evMaskSetNotMatchClass_function']){
				C.evMaskSetNotMatchClass_function=function(wrap_id){
					$('#'+wrap_id).removeClass(hash.matchClass).removeClass(hash.editingClass).addClass(hash.notmatchClass);
				}
				C.evMaskSetNotMatchClass_timers={};
			}
			$wrap.bind('check4changes', function(evt){
				$field=$(this).find('input,textarea').eq(0);
				if($field.length==0){
					$field=$(this);
				}
				back_value=$field.attr('data-backvalue');
				actual_value=$field[0].value;
				bool_clear_timeout=true;
				if(actual_value){
					if(actual_value.search(hash.re)>=0){
						$(this).removeClass(hash.editingClass).removeClass(hash.notmatchClass).addClass(hash.matchClass);
					}else if(actual_value!=back_value){
						$(this).removeClass(hash.matchClass).removeClass(hash.notmatchClass).addClass(hash.editingClass);
						$field.attr({'data-backvalue':actual_value});
					}else{
						if(!C.evMaskSetNotMatchClass_timers[wrap_id]){
							C.evMaskSetNotMatchClass_timers[wrap_id]=setTimeout('C.evMaskSetNotMatchClass_function("'+wrap_id+'")', hash.remainEditing);
						}
						bool_clear_timeout=false;
					}
				}else{
					$(this).removeClass(hash.matchClass).removeClass(hash.notmatchClass).removeClass(hash.editingClass);
				}
				if(C.evMaskSetNotMatchClass_timers[wrap_id] && bool_clear_timeout){
					clearTimeout(C.evMaskSetNotMatchClass_timers[wrap_id]);
					delete(C.evMaskSetNotMatchClass_timers[wrap_id]);
				}
			})
			setInterval('$("#'+wrap_id+'").trigger("check4changes")', hash.checkInterval);
		})
	}
})
