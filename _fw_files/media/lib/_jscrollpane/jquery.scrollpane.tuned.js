		...
		function()
		{
			...
			if($this.get(0).scrollHeight >= $this.get(0).offsetHeight){
				if (jQuery(this).parent().is('.jScrollPaneContainer')) {
					var currentScrollPosition = settings.maintainPosition ? ($this.offset().top - jQuery(this).parent().offset().top) : 0;
				...
				if (percentInView < .99) {
					...
					{
						if(_animateToHash){// && typeof $.easing[_animateToHash.easing]=='function'
							_animateToHash.t+=settings.animateInterval;
							var diff=$.easing[_animateToHash.easing](_animateToHash.t/_animateToHash.d, _animateToHash.t, _animateToHash.b, _animateToHash.c, _animateToHash.d);
							if (Math.abs(_animateToHash.c-diff)>.5) {
								positionDrag(diff);
							} else {
								positionDrag(_animateToHash.c);
								eraseAnimation();
							}
						}
					}
					var eraseAnimation = function()
					{
						if (_animateToHash && _animateToHash.timer) {
							clearInterval(_animateToHash.timer);
							_animateToHash=null;
						}
					};
					...
					{
						var pos=0;
						if (typeof hash.to=='string') {
							$e = jQuery(hash.selector, this);
							if (!$e.length) return;
							pos=$e.offset().top - $(this).offset().top;
						}else if(typeof hash.to=='object'){
							pos=hash.to.offset().top - $(this).offset().top;
						}else if(typeof hash.to=='number'){
							pos=hash.to;
						}
						//сбрасываем анимацию, если была
						eraseAnimation();
						//определяем куда нужно прокрутить движок
						var drag = -pos/(paneHeight-contentHeight) * maxY;
						if (!hash.prevent || settings.animateTo) {
							//закладываем новую анимацию
							_animateToHash={
								easing: (hash.easing || jQuery.easing.def), // если easing не определен, то определяем easing по умолчанию из jquery.easing.js
								t: 0,//start time
								b: dragPosition, //begin value
								c: drag, //end value
								d: (hash.msec || 1000), //если duration не определен, то по умолчанию выбираем секунду 
								timer: setInterval(animateToPosition, settings.animateInterval)
							};
						} else {
							//анимация не нужна
							positionDrag(drag);
					...
					$this[0].scrollBy = function(delta)
					{
						...
						scrollTo({to: currentPos+delta});
					};
					
					initDrag();
					
					scrollTo({to:-currentScrollPosition, prevent:true});
					
					var add_to_active=true;
					for(var i=0; i<jQuery.jScrollPane.active.length; i++){
						if(jQuery.jScrollPane.active[i]==$this[0]){
							add_to_active=false;
						}
					}
					if(add_to_active){
