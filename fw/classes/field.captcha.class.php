...
class CaptchaField extends Field {
	function constantProps(){
		...
		*/
		...
		$this->maxlength=1;
		/*
		так делать нельзя, потому что в этом случае невозможно сделать поле модели необязательным - 
		в _models.php при объявлении поля просто не будет добавляться атрибут blank
		кроме того, каптчу нужно вызывать только в методе validate, причем самой последней,
		для того, чтобы другие проверки осуществлялись перед проверкой на робота.
		$this->editable=(bool)($GLOBALS['path'][1]!='admin');//в админке вообще не выводим это поле
		$this->blank=(bool)($GLOBALS['path'][1]=='admin');
		*/
	}
