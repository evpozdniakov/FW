...
class Admin{
	function autopage(){//показывает страницу в зависимости от URL
		if(p2v('action')=='login'){
			...
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/login.php');
			$this->body=$result;
			//определяем весь документ
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/doctype.php');
			...
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/synchro_report.php');
			...
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/3zones.php');
			$this->body=$result;
			//определяем весь документ
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/doctype.php');
	...
	function getDomainsListHTML(){
		...
		if(count($structure_items)>1){
			...
			include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/domains_list.php');
		}
	...
	function getModelsListHTML(){//возвращает HTML-код списка моделей
		...
		//вызываем шаблон вывода списка моделей
		include($_SERVER['DOCUMENT_ROOT'].'/admin/fw/classes/templates/admin/models_list.php');
		return $result;
	}
	...
	function &createModelObj($model_name){//создает объект модели с именем $model_name
		...
		*/
		...
		if(!class_exists($model_name)){
			...
			$scriptLocation=$_SERVER['DOCUMENT_ROOT'].'/admin/models/'.$model_name.'/_models.php';
	...
	function try2login(){
		...
		if($dbq->rows>1){
			...
			if(false
				...
				$unix_mtime=filemtime($_SERVER['DOCUMENT_ROOT'].$phplog_dir.$phplog_file);
